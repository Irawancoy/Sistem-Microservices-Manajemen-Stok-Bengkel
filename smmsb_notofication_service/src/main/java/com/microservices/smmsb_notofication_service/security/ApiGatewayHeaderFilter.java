package com.microservices.smmsb_notofication_service.security;
...
public class ApiGatewayHeaderFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (path.startsWith("/notification-service/swagger-ui") ||
                path.startsWith("/notification-service/v3/api-docs")) {
            filterChain.doFilter(request, response);
            return;
        }

        // Only check gateway header for /api/v1/notifications/ endpoint
